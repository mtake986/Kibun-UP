...
const CommentUpdateForm = ({ comment, setIsUpdateMode, proposalId }: Props) => {
  const { updateComment } = useProposalComment();
  ...
  async function onSubmit(values: z.infer<typeof commentSchema>) {
    setIsPending(true);
    try {
      await updateComment(comment.id, values.comment, proposalId);
    } catch (error) {
      // 送信失敗したらalertで表示
      displayToast({
        text: "ERROR: " + error,
